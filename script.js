...
function generateQRCode() {
    ...
    const siteURL = 'https://fable-body.vercel.app/';
    
    // Generate QR code with custom styling
    QRCode.toCanvas(qrContainer, siteURL, {
        width: 180,
        height: 180,
        color: {
            dark: '#ff1493',  // Hot pink color for QR code
        ...
        if (error) {
            ...
            qrContainer.innerHTML = '<p style="color: #ff69b4;">QR Code unavailable</p>';
        } else {
...
function setupQRButtons() {
    ...
    const customBtn = document.getElementById('generateCustomQR');
    ...
    if (customBtn) {
        customBtn.addEventListener('click', openCustomQRGenerator);
    }
}
...
function downloadQRCode() {
    ...
    if (!canvas) {
        showNotification('🌸 QR Code not ready yet! Please wait a moment ✨');
        return;
    }
    
    // Create download link
    const link = document.createElement('a');
    link.download = 'FableBody-QRCode.png';
    ...
    showNotification('💖 QR Code downloaded! Share the FableBody love ✨');
}

function openCustomQRGenerator() {
    // Open the custom QR generator page
